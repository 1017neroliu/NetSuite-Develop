...
(function initScripting() {
	...
	callRecordScript = (function () {

		var fetchRecordScriptHandler = (function() {

			var recordScriptingHandler;
			function doFetching(debug) {

				if (recordScriptingHandler)
					return recordScriptingHandler;

				if (typeof _eventHandlerModules !== 'undefined') {
					includeN(debug);
					...
					require.setInternal(true);
					try {
						require(['N/currentRecord', 'N/util/recordScripting'],
							function (currentRecordModule, recordScriptingModule) {
								recordScriptingHandler = recordScriptingModule.create({
									record: currentRecordModule.get(),
									eventHandlerModules: _eventHandlerModules,
									scriptingContext: typeof _scriptingContext !== 'undefined' ? _scriptingContext : null
								});
							});
					}
					finally {
		...
		function doCallRecordScripting()
		{
			if (!!window && !!window.location && !!window.location.href && /(\?|&)withcurrentrecord\b/.test(window.location.href))
				return true;

			var returnResult = true;
			var scriptingHandler = fetchRecordScriptHandler();
			if (scriptingHandler)
				returnResult = scriptingHandler.runStaticEvent.apply(undefined, arguments);
			return returnResult;
		}

		return doCallRecordScripting;
	})();


	runClientScript = (function () {
		function doRunClientScript(trigger, args)
		{
			var args = args || [];
			var isValid = true;
								...
								if (validCall) {
									...
									if (typeof(currentClientScriptFunction) === "function") {
										...
										try {
											...
											isValid = alignValidationResult(currentClientScriptFunction.apply(null, currentArguments));
...
var fetchClientScripts = (function ()
{
	...
	function doneLoading() {
		if (typeof _clientScriptContext === "undefined" || typeof moduleKeeper === "undefined")
		{
			return false;
		}

		var loadedCount = 0;
		for (var i = 0; i < moduleKeeper.length; i++) {
			if (!!moduleKeeper[i])
			{
				loadedCount++;
			}
		}
		return typeof _clientScriptContext !== 'undefined'
			//If we find an emty element we're not done loading.
			&& loadedCount === _clientScriptContext.superScriptList.length;
	}
...
function nlapiSaveRecord()
{
    ...
    {
        isValid = handleValidationResult(nsapiCallScript("saveRecord", "internal", document.forms['main_form'].elements.nsapiSR.value));
        if (!isValid)
            return false;
    }
    isValid = isValid && handleValidationResult(callClientScript("saveRecord"));
    return isValid;
}
...
function nlapiValidateField(type, fldnam, linenum, column, optionswin)
{
    ...
    try
    {
        ...
        {
            isValid = handleValidationResult(nsapiCallScript("validateField", "internal", document.forms['main_form'].elements.nsapiVF.value, [type, fldnam, linenum, column]));
            if (!isValid)
                return false;
        }
        isValid = isValid && handleValidationResult(callClientScript("validateField", [type, fldnam, linenum, column]));
        return isValid;
    }
    finally
    {
...
function nlapiValidateDelete(type)
{
    try
    {
        ...
        {
            isValid = handleValidationResult(nsapiCallScript("validateDelete", "internal", document.forms['main_form'].elements.nsapiVD.value, [type]));
        }
        isValid = isValid && handleValidationResult(callClientScript("validateDelete", [type]));
        return isValid;
    }
    finally
    {
...
function nlapiValidateInsert(type)
{
    try
    {
        ...
        {
            isValid = handleValidationResult(nsapiCallScript("validateInsert", "internal", document.forms['main_form'].elements.nsapiVI.value, [type]));
        }
        isValid = isValid && handleValidationResult(callClientScript("validateInsert", [type]));
        return isValid;
    }
    finally
    {
...
function nlapiValidateLine(type)
{
    try
    {
        ...
        {
            isValid = handleValidationResult(nsapiCallScript("validateLine", "internal", document.forms['main_form'].elements.nsapiVL.value, [type]));
            if (!isValid)
                return false;
        }
        isValid = isValid && handleValidationResult(callClientScript("validateLine", [type]));
        return isValid;
    }
    finally
    {
