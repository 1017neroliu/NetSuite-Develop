...
function createCreditRefund(dataIn) {
	...
	var scriptId = context.getScriptId();
		...
		if (dataIn) {
			...
			creditmemoRec.setFieldValue('custbody_top_up_number',dataIn.topUpNumber);//充值单单号
			creditmemoRec.setFieldValue('custbody24', dataIn.vipId);//vip ID
			if(dataIn.memo){
			...
			for (var x = 0; x < source.length; x++) {
				...
				creditmemoRec.setCurrentLineItemValue('item', 'item', dataItemId);//货品ID
				creditmemoRec.setCurrentLineItemValue('item', 'taxcode', dataTaxcode);//货品taxcode
			...
			var topUpNumber = creditmemoRec.getFieldValue('custbody_top_up_number');
			var vipId = creditmemoRec.getFieldValue('custbody24');
