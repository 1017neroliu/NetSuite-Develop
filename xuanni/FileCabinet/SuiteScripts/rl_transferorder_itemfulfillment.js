			...
			if (dataIn) {
				...
				try {
					...
					var fLsearch = nlapiSearchRecord('location', null,
							[ new nlobjSearchFilter('custrecord_locationid',
									null, 'is', dataIn.fromLocationCode) ]);
					if (fLsearch != null) {
						var fromLocation2 = fLsearch[0].getId();
					}
					var tLsearch = nlapiSearchRecord('location', null,
							[ new nlobjSearchFilter('custrecord_locationid',
									null, 'is', dataIn.toLocationCode) ]);
					if (tLsearch != null) {
...
//					transferOrderRec.setFieldValue('orderstatus', 'B');//状态设置为PENDING RECEIPT（已通过审批）
				...
				try {
					...
					var itemfulfillmentRec = nlapiTransformRecord(
							'transferorder', transferOrderId, 'itemfulfillment');
